...
echo 'alias turchain="$HOME/core-bridgechain/packages/core/bin/run"' >> ~/.bashrc

rm -rf "$HOME/core-bridgechain"
git clone "https://github.com/TURchain/core" "$HOME/core-bridgechain" || FAILED="Y"
if [ "$FAILED" == "Y" ]; then
    echo "Failed to fetch core repo with origin 'https://github.com/TURchain/core'"
...
rm -rf "$HOME/.config/@turchain"
rm -rf "$HOME/.config/@turchain"
rm -rf "$HOME/.config/turchain-core"
